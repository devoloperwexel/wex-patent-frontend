...
const SessionCard: React.FC<SessionCardProps> = ({
  ...
  const lng =
    languages[language.toLowerCase() as keyof typeof languages].dashboard1
      .session;

  const {
    availableSessions,
    totalSessions: totalSessionLabel,
    usedSessions: usedSessionLabel,
    ...
    <div className="bg-white rounded-2xl px-6 py-4 flex flex-col justify-center items-center space-y-3 shadow-md h-[300px]">
      ...
      <div className="flex flex-col items-start space-y-2">
        <p className="text-[16px] font-semibold text-black">
          {totalSessionLabel}: {totalSessions}
        </p>
        <p className="text-[16px] font-semibold text-black">
          {usedSessionLabel}: {usedSessions}
        </p>
      </div>
      <button
